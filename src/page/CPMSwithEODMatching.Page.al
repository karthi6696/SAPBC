page 85023 "CPMS with EOD Matching"
{
    ApplicationArea = All;
    Caption = 'CPMS Matching';
    PageType = ListPart;
    SourceTable = TTS_ARAP;
    SourceTableView = sorting("EOD Matching Status", "EOD Processed Date Time") order(descending);
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                field("Matching ID"; Rec."EOD Matching ID")
                {
                    ToolTip = 'Specifies the value of the Matching ID field.', Comment = '%';
                    StyleExpr = StyleExp;

                }
                field("Matching Date Time"; Rec."EOD Processed Date Time")
                {
                    ToolTip = 'Specifies the value of the Matching Date Time field.', Comment = '%';
                    StyleExpr = StyleExp;

                }
                field("Matching Status"; Rec."EOD Matching Status")
                {
                    ToolTip = 'Specifies the value of the Matching Status field.', Comment = '%';
                    StyleExpr = StyleExp;
                }
                field("EOD Match Details"; Rec."EOD Match Details")
                {
                    ToolTip = 'Specifies the value of the EOD Match Details field.', Comment = '%';
                    StyleExpr = StyleExp;
                }
                field("EOD Matched By"; Rec."EOD Matched By")
                {
                    ToolTip = 'Specifies the value of the EOD Matched By field.', Comment = '%';
                    StyleExpr = StyleExp;

                }
                field("EOD Match Type"; Rec."EOD Match Type")
                {
    ...
    actions
    {
        area(Processing)
        {
            action("Show Linked EOD")
            {
                ...
                begin
                    if Rec."EOD Matching Status" IN [Rec."EOD Matching Status"::Matched, Rec."EOD Matching Status"::Error] then begin
                        EOD.Reset();
                        EOD.SetRange("Matching ID", Rec."EOD Matching ID");
                        EOD.SetRange("Matching Status", Rec."EOD Matching Status");
                        Page.RunModal(page::"EOD Matching", EOD);
                    end;
                end;
            }
            action("Force Match")
            {
                ...
                begin
                    ...
                    CurrPage.SetSelectionFilter(CPMSRecords);
                    GLSetup.get();
                    MatchID := Noseries.GetNextNo(GLSetup."LOB-CPMS Matching No. Series");
                    if CPMSRecords.FindSet(true) then
                        repeat
            ...
            action("Remove Match")
            {
                ...
                begin
                    ...
                    CPMSRecords.SetRange("EOD Match Type", CPMSRecords."EOD Match Type"::Force);
                    if CPMSRecords.FindSet() then
                        repeat
                            CPMSRecords.Validate("EOD Matching Status", CPMSRecords."EOD Matching Status"::Unmatched);
                            CPMSRecords.Modify();
            ...
            action("CPMS Export")
            {
                Caption = 'Unmatched Items CPMS';
                Image = ExportElectronicDocument;
                ApplicationArea = All;

                trigger OnAction()
                var
                    CPMSExport: Codeunit "CPMS Export";
                begin
                    CPMSExport.CreateCPMSCSVFile();
                end;
            }
        }
    }
    ...
    begin
        Clear(StyleExp);
        case true of
            Rec."EOD Matching Status" = Rec."EOD Matching Status"::Matched:
                StyleExp := 'Favorable';
            Rec."EOD Matching Status" = rec."EOD Matching Status"::Error:
                StyleExp := 'UnFavorable';
        end;
    end;

    procedure ToggleEODMatchedFilter(SetFilterOn: Boolean)
    begin
        if SetFilterOn then begin
            rec.SetRange("EOD Matching Status");
            Rec.SetRange("EOD Matching Status", Rec."EOD Matching Status"::Unmatched)
        end else
            Rec.SetRange("EOD Matching Status");
    ...
    begin
        rec.SetRange("EOD Matching Status");
        Rec.SetRange("EOD Matching Status", Rec."EOD Matching Status"::Error);
        CurrPage.Update();
    end;
    ...
    begin
        rec.SetRange("EOD Matching Status");
        Rec.SetRange("EOD Matching Status", Rec."EOD Matching Status"::Matched);
        CurrPage.Update();
    end;
    ...
    begin
        rec.SetRange("EOD Matching Status");
        Rec.SetRange("EOD Matching Status", Rec."EOD Matching Status"::Matched);
        Rec.SetRange("EOD Match Type", Rec."EOD Match Type"::Force);
        CurrPage.Update();
    end;

    procedure GetSelectedRecords(var SelectedCPMSEOD: Record TTS_ARAP temporary)
    var
        CPMSEOD: Record TTS_ARAP;
    begin
        CurrPage.SetSelectionFilter(CPMSEOD);
        if CPMSEOD.FindSet() then
            repeat
                SelectedCPMSEOD := CPMSEOD;
                SelectedCPMSEOD.Insert();
            until CPMSEOD.Next() = 0;
    end;
