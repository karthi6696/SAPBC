page 85030 "DVSA Navigate"
{
    ...
    PageType = Worksheet;
    SaveValues = true;
    SourceTable = "DVSA Document Entry";
    ...
    procedure SetDoc(PostingDate: Date; DocNo: Code[100])
    begin
        NewDocNo := DocNo;
    ...
    begin
        ...
        SetSource(0D, DVSADocumentType, DVSADocumentNo, 7, '');
    ...
    procedure SetDVSASource(var DVSADocType: Text; var DVSADocNo: Text)
    begin
        DVSADocumentType := DVSADocType;
        DVSADocumentNo := DVSADocNo;
    end;
    ...
    procedure InsertIntoDocEntry(var TempDocumentEntry: Record "DVSA Document Entry" temporary; DocTableID: Integer; DocTableName: Text; DocNoOfRecords: Integer)
    begin
        InsertIntoDocEntry(TempDocumentEntry, DocTableID, Enum::"Document Entry Document Type"::" ", DocTableName, DocNoOfRecords);
    end;

    procedure InsertIntoDocEntry(var TempDocumentEntry: Record "DVSA Document Entry" temporary; DocTableID: Integer; DocType: Enum "Document Entry Document Type"; DocTableName: Text; DocNoOfRecords: Integer)
    begin
        ...
        if ItemTrackingSearch() then
            ...
            case Rec."Table ID" of
                Database::TTS_SAP:
                    begin
                        TTSSAP.Reset();
                        TTSSAP.SetRange(PaymentReference, DocNoFilter);
                        Page.run(Page::"TTS Records", TTSSAP)
                    end;
                Database::TTS_ARAP:
                    begin
                        CPMS.reset;
                        CPMS.SetRange(ReceiptNumber, DocNoFilter);
                        Page.Run(Page::"CPMS Records", CPMS);
                    end;
                Database::"EOD Staging":
                    begin
                        EOD.Reset();
                        EOD.SetRange("Reference Number", DocNoFilter);
                        Page.Run(Page::"EOD Staging", EOD);
                    end;
    ...
    begin
        Rec.SetFilter("Document No.", DocNo); //  DJA
        DocNoFilter := Rec.GetFilter("Document No.");
        PostingDateFilter := Rec.GetFilter("Posting Date");
    end;
    ...
    local procedure FindTTSSapRecords()
    var
        TTSSap: Record TTS_SAP;
    begin
        TTSSap.Reset();
        TTSSap.SetRange(PaymentReference, DocNoFilter);
        InsertIntoDocEntry(Rec, Database::TTS_SAP, 'TTS SAP', TTSSap.Count);
    end;

    local procedure FindCPMSRecords()
    var
        TTSARAP: Record TTS_ARAP;
    begin
        TTSARAP.Reset();
        TTSARAP.SetRange(ReceiptNumber, DocNoFilter);
        InsertIntoDocEntry(Rec, Database::TTS_ARAP, 'CPMS', TTSARAP.Count);
    end;

    local procedure FindEODRecords()
    var
        EOD: Record "EOD Staging";
    begin
        EOD.Reset();
        EOD.SetRange("Reference Number", DocNoFilter);
        InsertIntoDocEntry(Rec, Database::"EOD Staging", 'EOD', EOD.Count);
    end;
    ...
    procedure ReturnDocumentEntry(var TempDocumentEntry: Record "DVSA Document Entry" temporary)
    begin
    ...
    local procedure OnAfterFindCostEntries(var DocumentEntry: Record "DVSA Document Entry"; DocNoFilter: Text; PostingDateFilter: Text)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterFindRecords(var DocumentEntry: Record "DVSA Document Entry"; DocNoFilter: Text; PostingDateFilter: Text)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterFindPostedDocuments(var DocNoFilter: Text; var PostingDateFilter: Text; var DocumentEntry: Record "DVSA Document Entry")
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterFindPurchRcptHeader(var DocumentEntry: Record "DVSA Document Entry"; var PurchRcptHeader: Record "Purch. Rcpt. Header"; DocNoFilter: Text; PostingDateFilter: Text)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterNavigateFindExtRecords(var DocumentEntry: Record "DVSA Document Entry"; ContactType: Enum "Navigate Contact Type"; ContactNo: Code[250]; ExtDocNo: Code[250]; var FoundRecords: Boolean)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterNavigateFindRecords(var DocumentEntry: Record "DVSA Document Entry"; DocNoFilter: Text; PostingDateFilter: Text; var NewSourceRecVar: Variant; ExtDocNo: Code[250]; HideDialog: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterNavigateFindTrackingRecords(var DocumentEntry: Record "DVSA Document Entry"; SerialNoFilter: Text; LotNoFilter: Text)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnAfterNavigateShowRecords(TableID: Integer; DocNoFilter: Text; PostingDateFilter: Text; ItemTrackingSearch: Boolean; var TempDocumentEntry: Record "DVSA Document Entry" temporary; SalesInvoiceHeader: Record "Sales Invoice Header"; SalesCrMemoHeader: Record "Sales Cr.Memo Header"; PurchInvHeader: Record "Purch. Inv. Header"; PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr."; ServiceInvoiceHeader: Record "Service Invoice Header"; ServiceCrMemoHeader: Record "Service Cr.Memo Header"; ContactType: Enum "Navigate Contact Type"; ContactNo: Code[250]; ExtDocNo: Code[250])
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnFindExtRecordsOnBeforeFormUpdate(var Rec: Record "DVSA Document Entry"; var SalesInvHeader: Record "Sales Invoice Header"; var SalesCrMemoHeader: Record "Sales Cr.Memo Header")
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnFindRecordsOnAfterSetSource(var DocumentEntry: Record "DVSA Document Entry"; var PostingDate: Date; var DocType2: Text[100]; var DocNo: Code[20]; var SourceType2: Integer; var SourceNo: Code[20]; var DocNoFilter: Text; var PostingDateFilter: Text; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFindRecordsOnOpenOnAfterSetDocuentFilters(var Rec: Record "DVSA Document Entry"; var DocNoFilter: Text; var PostingDateFilter: Text; ExtDocNo: Code[250]; NewSourceRecVar: Variant)
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnBeforeFindRecordsSetSources(var DocumentEntry: Record "DVSA Document Entry"; DocNoFilter: Text; PostingDateFilter: Text; ExtDocNo: Text; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFindRecordsOnBeforeMessagePostingDateFilter(DocumentEntry: Record "DVSA Document Entry"; PostingDateFilter: Text; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterFindLedgerEntries(var DocumentEntry: Record "DVSA Document Entry"; DocNoFilter: Text; PostingDateFilter: Text)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterOnInit(var DocumentEntry: Record "DVSA Document Entry");
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePrint(var Rec: Record "DVSA Document Entry"; SearchBasedOn: Enum "Navigate Search Type"; var TempRecordBuffer: Record "Record Buffer"; var ItemTrackingFilters: Record Item; DocNoFilter: Text; PostingDateFilter: Text; var IsHandled: Boolean);
    begin
    end;
}
