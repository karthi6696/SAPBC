report 85010 "Matched CPMS EOD"
{
    ...
    dataset
    {
        ...
        {
            RequestFilterFields = "EOD Match Type", ReceiptDate;
        ...
        {
            RequestFilterFields = "Match Type", "Transaction Date";
            ...
            begin
                // "EOD Staging".SetRange("Match Type", "EOD Staging"."Match Type"::Automatic);
                // //"EOD Staging".SetRange("Transaction Date", CalcDate('<-CM>', Today), CalcDate('<CM>', Today));

                // CPMSStaging.SetRange("EOD Match Type", CPMSStaging."EOD Match Type"::Automatic);
                // //CPMSStaging.SetRange(ReceiptDate, CalcDate('<-CM>', Today), CalcDate('<CM>', Today));

                if not guiAllowed then begin
                    "EOD Staging".SetRange("Matching Status", "EOD Staging"."Matching Status"::Matched);
...
000000T);
    ...
    rendering
    {
        ...
        {
            type = Excel;
            LayoutFile = 'MatchedCPMSEOD.xlsx';
        }
    }
